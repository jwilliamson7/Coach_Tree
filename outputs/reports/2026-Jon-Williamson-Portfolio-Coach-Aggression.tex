...
The coaching data comes from career histories scraped from Pro Football Reference, covering the 185 NFL coaches active between 2006 and 2024. For each coach, we have complete position-by-position career timelines, allowing us to classify coaches by background (offensive vs defensive vs other) and construct mentor-protégé relationships based on overlapping team assignments, matched on a year-aware canonical franchise key so that relocations are handled correctly. Coach backgrounds are determined by analyzing their coordinator and position coach experience prior to becoming head coaches, using pattern matching on role titles to identify offensive versus defensive specialization. A coordinator is considered a protégé of a head coach if they worked together for at least one season, yielding 1{,}467 documented coaching relationships over this period.
...
We apply a single global Benjamini-Hochberg family spanning all 101 hypothesis tests in the study, preferring the wild cluster bootstrap p-value for small-cluster subgroups, the percentile clustered p-value for other repeated-measures tests, and the conventional p-value otherwise. This is the most conservative, least forking-path-prone choice. Under this family the Benjamini-Hochberg threshold falls at $p \leq 0.024$, below which 65 of the 101 tests lie. We treat this threshold as one decision rule rather than a sharp boundary and report both clustered and conventional p-values throughout.
...
\textbf{Variance decomposition.} Across every gene, coach identity explains far more variance than team identity. For composite offensive aggression, coach identity accounts for 51\% of the variance against 13\% for team and 19\% for season; the same pattern holds for shotgun (coach 58\%, team 12\%), tempo (53\% vs.\ 10\%), and the offensive aggression components (Table~\ref{tab:variance}). A coach's genes are roughly four to five times more a function of who they are than of where they coach.

\textbf{The gene travels with the coach.} A sharper test exploits coaching changes. Among the 32 coaches who changed teams during our window, the gene a coach exhibits at their new team is correlated with the gene they exhibited at their previous team: composite offensive aggression $r=0.61$ (95\% CI $[0.33, 0.78]$), shotgun $r=0.55$, tempo $r=0.54$, comparable to or stronger than the year-to-year persistence \emph{within} a single team (Section~\ref{sec:persistence}). A mover regression refines the attribution. Regressing a coach's new-team gene on both their prior-team gene and the new team's pre-arrival gene, we find the coach term large and well-determined (composite offensive aggression $\beta=0.53$, 95\% CI $[0.26, 0.83]$, $p<0.001$); the new team's pre-arrival tendency also carries predictive weight ($\beta=0.72$, $p=0.004$), consistent with teams hiring coaches whose style fits the building. The strong cross-team travel of the gene nonetheless shows that strategic identity is substantially carried by the coach.
...
\midrule
Composite Offensive Aggression & 0.51 & 0.13 & 0.61 [0.33, 0.78] \\
...
\midrule
Composite Offensive Aggression & Pederson, Staley, Arians & Jauron, Childress, Fox \\
...
\midrule
Composite Offensive Aggression & 0.189 & [0.10, 0.28] & $<$0.001** & 608 \\
Shotgun Formation & 0.142 & [0.05, 0.23] & 0.004** & 608 \\
Defensive Aggression & 0.151 & [0.03, 0.27] & 0.015* & 282 \\
Composite Tempo & 0.045 & [$-$0.04, 0.15] & 0.352 & 608 \\
\midrule
\multicolumn{5}{l}{\small Coach-clustered bootstrap p and 95\% CI; * $p<0.05$, ** $p<0.01$}\\
\multicolumn{5}{l}{\small Defensive aggression: 2016--2024; all others: 2006--2024}\\
\bottomrule
\end{tabular}
\end{table}

Three of the four genes are positively associated with performance. Composite offensive aggression shows the strongest correlation with WAR ($r=0.189$, $p<0.001$), followed by defensive aggression ($r=0.151$, $p=0.015$) and shotgun formation preference ($r=0.142$, $p=0.004$); all three are robust to coach clustering and the global false-discovery correction. Tempo shows little relationship ($r=0.045$, $p=0.35$).

These season-level correlations are conservative. As noted in the Methods, a single-season WAR is only about one-quarter reliable, and measurement noise in the outcome attenuates a correlation toward zero, so these values are lower bounds. At the \emph{career} level, where a coach's mean WAR is far more reliable, the composite offensive aggression relationship strengthens to $r=0.303$ (95\% CI $[0.16, 0.45]$, $p<0.001$, $n=124$ coaches; $r=0.308$, $[0.14, 0.47]$, among the 87 coaches with at least three seasons), and shotgun to $r=0.230$ ($[0.04, 0.41]$, $p=0.010$); defensive aggression is positive but imprecise at the career level given its shorter window ($r=0.188$, $[0.02, 0.35]$, $p=0.10$, $n=79$). The season-level estimates are also robust to inverse-variance weighting by WAR precision (composite offensive aggression $r=0.211$, $[0.13, 0.29]$) and to dropping partial seasons ($r=0.211$), so they are not driven by a handful of noisy short tenures. Given that most coaching genes carry real performance signal, we turn to whether they transmit through coaching relationships, and whether the genes that transfer most strongly are the same ones that predict winning.
...
Shotgun formation preference shows clearer overall inheritance ($r=0.289$, $p<0.001$, $n=608$), several times stronger than composite offensive aggression ($r=0.080$), and the association holds under mentor clustering.
...
Figure~\ref{fig:inherit_type} visualizes the offensive aggression inheritance patterns split by mentor background.

\begin{figure}[H]
    \centering
    \includegraphics[width=0.9\textwidth]{../visualizations/inheritance/inheritance_by_mentor_type.png}
    \caption{Mentor-prot\'{e}g\'{e} offensive aggression correlations split by mentor background. Only offensive mentors (orange) show a clear positive slope, most visibly for 4th-down aggression. Defensive mentors (blue) show flat relationships across all components.}
...
\multicolumn{7}{l}{\small Defensive aggression ($n=17$) uses a wild cluster bootstrap p; the offensive transitions ($n=40$) use the coach-clustered bootstrap p; 95\% CIs are the percentile coach cluster bootstrap; * $p<0.05$, *** $p<0.001$}\\
\multicolumn{7}{l}{\small The two strongest results fall below the global FDR threshold ($p \leq 0.024$)}\\
\bottomrule
\end{tabular}
\end{table}

The results reinforce the mentor-prot\'{e}g\'{e} findings. Shotgun formation preference, the most visible schematic element of offensive identity, shows strong direct transmission ($r=0.674$, $p<0.001$), consistent with the OC mentor-prot\'{e}g\'{e} correlation ($r=0.399$, Table~\ref{tab:shotgun_inherit}). When an offensive coordinator runs shotgun-heavy formations, about 82\% of the time their subsequent head-coaching tenure shows the same tendency. Matt Nagy, for instance, carried a shotgun-heavy identity from his Kansas City coordinator years to his Chicago head-coaching tenure, and Jason Garrett a below-average one from his Dallas coordinator years through his decade as the team's head coach.

Defensive aggression also shows a positive association ($r=0.570$; $p=0.020$ from a wild cluster bootstrap appropriate to the small sample), though with only $n=17$ transitions, limited by defensive tracking data from 2016 onward, this is suggestive rather than definitive. Dan Quinn, for example, carried an aggressive defense from his Dallas coordinator tenure to Washington, and Matt Patricia a conservative one from New England to Detroit.

Tempo shows a moderate point estimate that is not statistically reliable at this sample size ($r=0.318$; coach-clustered bootstrap $p=0.23$, naive $p=0.046$), so we do not read it as dependable coordinator-to-HC inheritance. Offensive Aggression shows essentially none ($r=-0.008$, $p=0.93$), consistent with the career-average analysis. Of these four, the shotgun and defensive-aggression associations fall below the global Benjamini-Hochberg threshold of $p \leq 0.024$.
...
The composite offensive aggression-WAR correlation ($r=0.189$, Table~\ref{tab:gene_war}) masks a dynamic temporal story. Table~\ref{tab:overall_war} presents the component-level relationships pooling across all seasons from 2006-2024.
...
Since these offensive aggression measures focus on offensive decisions (going for it on 4th down, passing vs running, deep pass targeting, two-point attempts), we examine whether offensive-minded head coaches show different offensive aggression-performance relationships than defensive coaches making offensive decisions.

\begin{table}[H]
\caption{Offensive Aggression vs. WAR by Coach Type}
\label{tab:type_war}
\scriptsize
\begin{tabular}{lcccc}
\toprule
...
Both offensive ($r=0.211$, $p<0.001$) and defensive ($r=0.184$, $p=0.003$) coaches show positive relationships of similar magnitude between composite offensive aggression and WAR. The two backgrounds are comparable, indicating that offensive aggression relates to performance regardless of whether a coach comes from the offensive or defensive side of the ball.
...
Offensive coaches show stronger year-to-year persistence than defensive coaches for the specific components, most clearly 4th down decisions ($r=0.639$ vs $r=0.352$). The gap is substantial: offensive coaches explain 41\% of next year's 4th down aggression from this year's value, while defensive coaches explain only 12\%. For composite offensive aggression, however, the gap is small ($r=0.550$ vs $r=0.518$, difference=0.032): the offensive-defensive divergence is concentrated in the 4th-down and pass-heavy components rather than in overall offensive aggression.
...
Our analyses reveal a distinction between two types of coaching DNA, though not as clean as a simple binary. \emph{Schematic identity}, meaning formation preferences and defensive packages, transfers through offensive coaching trees (shotgun OC: $r=0.399$, $p<0.001$) and persists through promotion (shotgun: $r=0.674$, $p<0.001$; defensive aggression: $r=0.570$, suggestive at $n=17$). \emph{Situational decision-making}, the in-game risk choices, predicts performance (offensive aggression: $r=0.189$, $p<0.001$, strengthening to $r=0.30$ at the career level) but is not transmitted through coaching trees ($r=0.080$, $p=0.24$).

Shotgun preference and defensive aggression bridge both categories. They transfer through coaching relationships \emph{and} predict performance (shotgun: $r=0.142$, $p=0.004$; defensive aggression: $r=0.151$, $p=0.015$). This suggests that some schematic choices are not purely stylistic, since they carry competitive implications. Transmission also appears domain-specific, with only offensive coordinators transmitting offensive schematic identity (shotgun DC: $r=0.096$, near zero), consistent with coaching DNA propagating through expertise rather than mere exposure.
...
The coaching tree narrative shapes NFL hiring decisions, but our data sit uneasily with its premises. Mentor WAR shows at most a weak association with prot\'{e}g\'{e} WAR ($r=0.124$, $p=0.074$), giving little indication that working under a successful coach makes a coach successful. Composite offensive aggression shows little transmission ($r=0.080$). For situational decisions, the premises of pedigree-based hiring find little support here.
