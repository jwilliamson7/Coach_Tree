...
Finally, career-average gene leaders match public reputation
(Table~\ref{tab:facevalidity}): the most aggressive coaches by career composite are
Brandon Staley and Doug Pederson, fourth-down aggression is led by Dan Campbell,
shotgun by Nick Sirianni and Chip Kelly, and tempo by Chip Kelly, each two to seven
standard deviations from the average coach and each a name a knowledgeable observer
would supply. Together with the repeatabilities, these four lines establish that the
phenotypes are stable properties of the coach.

\begin{table}[t]
\centering
\caption{Face validity: career-average gene leaders and laggards (coaches with
$\geq 3$ seasons, $n=89$; $z$ = standard deviations from the average qualifying
coach).}
...
Gene & Most ($z$) & Least ($z$) \\
\midrule
Comp.\ offensive aggression & Staley (+2.7), Pederson (+2.6), Koetter (+2.3) & Jauron ($-$2.1), Childress ($-$2.0), Ryan ($-$1.8) \\
Fourth-down & Campbell (+3.2), LaFleur (+3.1), Staley (+2.9) & Holmgren ($-$1.4), Dungy ($-$1.3), McCoy ($-$1.3) \\
Shotgun & Sirianni (+2.4), Kingsbury (+2.4), Kelly (+2.3) & Holmgren ($-$3.1), Nolan ($-$2.3), Marinelli ($-$1.9) \\
Tempo & Kelly (+6.7), Kingsbury (+3.1), Spagnuolo (+1.5) & Nolan ($-$1.4), LaFleur ($-$1.3), Gailey ($-$1.3) \\
Defensive aggression & Arians (+2.0), Daboll (+1.9), Flores (+1.8) & Lynn ($-$2.5), Reich ($-$1.8), LaFleur ($-$1.4) \\
\bottomrule
\end{tabular}
\end{table}
...
First, phylogenetic signal on the mentor network (Table~\ref{tab:moran}). Moran's
