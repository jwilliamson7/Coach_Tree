...
class PassRushDataProcessor:
    ...
    def encode_categorical_features(self, df: pd.DataFrame, feature_names: List[str],
        ...
        if 'rush_target' in non_numeric_cols:
            non_numeric_cols.remove('rush_target')
...
def main():
    ...
    try:
        ...
        feature_data_encoded = processor.encode_categorical_features(
            feature_data, feature_names + ['rush_target'], fit=True)

        X = feature_data_encoded.drop('rush_target', axis=1)
        y = feature_data_encoded['rush_target'].astype(float)
