...
class NoHuddleDataProcessor:
    ...
    def encode_categorical_features(self, df: pd.DataFrame, feature_names: List[str],
        ...
        if 'is_no_huddle' in non_numeric_cols:
            non_numeric_cols.remove('is_no_huddle')
...
def main():
    ...
    try:
        ...
        feature_data_encoded = processor.encode_categorical_features(
            feature_data, feature_names + ['is_no_huddle'], fit=True)

        X = feature_data_encoded.drop('is_no_huddle', axis=1)
        y = feature_data_encoded['is_no_huddle'].astype(int)
