...
class RunPassDataProcessor:
    ...
    def encode_categorical_features(self, df: pd.DataFrame, feature_names: List[str], 
        ...
        # Remove is_pass from non-numeric check if present
        if 'is_pass' in non_numeric_cols:
            non_numeric_cols.remove('is_pass')
...
def main():
    ...
    try:
        ...
        feature_data_encoded = processor.encode_categorical_features(feature_data, feature_names + ['is_pass'], fit=True)
        
        # Separate features and target
        X = feature_data_encoded.drop('is_pass', axis=1)
        y = feature_data_encoded['is_pass'].astype(int)
